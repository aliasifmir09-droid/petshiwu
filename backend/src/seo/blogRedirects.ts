/**
 * Thin neighborhood delivery posts 301 to the five borough hubs.
 * Shared by the HTTP middleware, route classifier, and sitemap so Google
 * is not asked to recrawl URLs that only redirect.
 */
...
export const redirectedBlogHubPath = (slug: string): string | null => {
  const hub = BLOG_REDIRECTS[slug];
  return hub ? `/learning/${hub}` : null;
};

...
export const getProducts = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (isAdminRequest) {
      ...
      selectFields = 'name slug images cloudinaryImage bunnyImage basePrice compareAtPrice averageRating totalReviews brand category petType isActive isFeatured inStock totalStock variants createdAt updatedAt tags';
    } else if (isFeaturedQuery) {
      // Featured products for home page - minimal fields
      selectFields = 'name slug images cloudinaryImage bunnyImage basePrice compareAtPrice averageRating totalReviews brand category petType isFeatured inStock totalStock variants';
    } else {
      ...
      selectFields = 'name slug shortDescription images cloudinaryImage bunnyImage basePrice compareAtPrice averageRating totalReviews brand category petType isFeatured inStock totalStock variants tags createdAt';
    }
...
export const getProductsCursor = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const products = await Product.find(baseQuery)
      ...
      .select('name slug images cloudinaryImage bunnyImage basePrice averageRating totalReviews brand category petType inStock totalStock')
...
export const getRelatedProducts = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const relatedProducts = await Product.find({
      ...
      .maxTimeMS(5000)
      .select('name slug images cloudinaryImage bunnyImage basePrice compareAtPrice averageRating totalReviews brand category petType isFeatured inStock totalStock variants')
      .populate({
        path: 'category',
...
export const getTrendingProducts = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const trendingProducts = await Product.find(query)
      .select('name slug images cloudinaryImage bunnyImage basePrice compareAtPrice averageRating totalReviews viewCount brand category petType')
      .populate({
        path: 'category',
        select: 'name slug'
      })

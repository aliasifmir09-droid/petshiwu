      ...
      <div className="container mx-auto px-6 lg:px-8 py-8 max-w-7xl">
        <div className="grid grid-cols-4 gap-x-10 gap-y-10">
          {categories.map((category, index) => (
            <div key={index}>
              <Link
                to={`/category/${encodeURIComponent(category.title.toLowerCase().replace(/\s+/g, '-'))}?petType=${petType === 'other' ? 'other-animals' : petType}`}
              ...
              <ul className="space-y-2">
                {category.items.map((item, itemIndex) => (
                  <li key={itemIndex}>
                    <Link
                      to={`/category/${encodeURIComponent(item.toLowerCase().replace(/\s+/g, '-'))}?petType=${petType === 'other' ? 'other-animals' : petType}`}

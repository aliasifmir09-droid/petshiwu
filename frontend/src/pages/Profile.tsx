...
function Confetti() {
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, zIndex: 5, pointerEvents: 'none', overflow: 'hidden' }}>
      {pieces.map((p, i) => (
        <div key={i} style={{ position: 'absolute', left: p.left, width: p.isTall ? 4 : 5, height: p.isTall ? 7 : 5, borderRadius: p.isCircle ? '50%' : 1, background: p.color, opacity: 0.9, animation: `confettiFall ${p.dur} linear ${p.delay} infinite` }} />
      ))}
    </div>
  );
}
...
function Balloons() {
  return (
    <div style={{ position: 'absolute', inset: 0, zIndex: 6, pointerEvents: 'none' }}>
      {[{ left: 4, delay: '0s', size: 18, dur: '3.8s' }, { left: undefined, right: 4, delay: '1.1s', size: 14, dur: '4.2s' }, { left: 36, delay: '2.2s', size: 13, dur: '3.5s' }].map((b, i) => (
        <span key={i} style={{ position: 'absolute', bottom: 0, ...(b.left !== undefined ? { left: b.left } : { right: (b as any).right }), fontSize: b.size, lineHeight: 1, animation: `balloonFloat ${b.dur} ease-in-out ${b.delay} infinite`, filter: 'drop-shadow(0 2px 4px rgba(0,0,0,.2))' }}>🎈</span>
      ))}
    </div>
  );
}

function PassportPetCard({ pet, isPrimary, onEdit, onDelete, onPhotoUpdate }: { pet: Pet; isPrimary?: boolean; onEdit: () => void; onDelete: () => void; onPhotoUpdate: (petId: string, url: string) => void; }) {
  ...
  const handlePhotoChange = useCallback(async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (json.success) { setLocalPhoto(json.data.photo); onPhotoUpdate(pet._id, json.data.photo); }
    } catch { } finally {
      setUploading(false);
  ...
  const speciesBg = { dog: 'linear-gradient(180deg,#0a1628 0%,#1a1208 55%,#7c2d12 100%)', cat: 'linear-gradient(180deg,#0c0820 0%,#1a0a2e 55%,#4a1272 100%)', bird: 'linear-gradient(180deg,#071520 0%,#0c1e32 55%,#1a4a72 100%)', fish: 'linear-gradient(180deg,#071a18 0%,#0c2e28 55%,#0d5a4a 100%)', reptile: 'linear-gradient(180deg,#071a07 0%,#0c2e0c 55%,#1a5a1a 100%)', 'small-animal': 'linear-gradient(180deg,#200820 0%,#3a0a3a 55%,#7a1472 100%)' }[pet.species] || 'linear-gradient(180deg,#080f20 0%,#0d1a3a 55%,#1e3a8a 100%)';
  const speciesOv = { dog: 'linear-gradient(180deg,transparent 30%,rgba(234,88,12,.45) 100%)', cat: 'linear-gradient(180deg,transparent 30%,rgba(147,51,234,.4) 100%)', bird: 'linear-gradient(180deg,transparent 30%,rgba(14,165,233,.4) 100%)', fish: 'linear-gradient(180deg,transparent 30%,rgba(13,148,136,.4) 100%)', reptile: 'linear-gradient(180deg,transparent 30%,rgba(34,197,94,.4) 100%)', 'small-animal': 'linear-gradient(180deg,transparent 30%,rgba(236,72,153,.4) 100%)' }[pet.species] || 'linear-gradient(180deg,transparent 30%,rgba(79,70,229,.35) 100%)';
  const neonGlow = sp.neon;
  const borderColor = { dog: 'rgba(234,88,12,.25)', cat: 'rgba(147,51,234,.25)', bird: 'rgba(14,165,233,.25)', fish: 'rgba(13,148,136,.25)', reptile: 'rgba(34,197,94,.25)', 'small-animal': 'rgba(236,72,153,.25)' }[pet.species] || 'rgba(99,102,241,.25)';

  return (
    <div style={{ borderRadius: 16, overflow: 'visible', ...(bday ? { padding: 2, background: 'linear-gradient(45deg,#f97316,#ec4899,#a855f7,#3b82f6,#22c55e,#eab308,#f97316)', backgroundSize: '300% 300%', animation: 'bdayBorder 2.5s ease infinite', boxShadow: '0 6px 28px rgba(249,115,22,.3)' } : { border: `1.5px solid ${borderColor}`, boxShadow: '0 3px 14px rgba(0,0,0,.08)' }) }}>
        ...
        <div style={{ display: 'flex', cursor: 'pointer', minHeight: 140 }} onClick={() => setExpanded(x => !x)}>
          <div style={{ width: 130, flexShrink: 0, position: 'relative', overflow: 'visible', minHeight: 140 }}>
            <div style={{ position: 'absolute', inset: 0, overflow: 'hidden', borderRadius: 0 }}>
              {localPhoto ? (
                <img src={localPhoto} alt={pet.petName} style={{ width: '100%', height: '100%', objectFit: 'cover', objectPosition: 'center 15%', display: 'block' }} />
              ) : (
                <>
                  ...
                  <div style={{ position: 'absolute', bottom: 30, left: 0, right: 0, zIndex: 3, textAlign: 'center', fontSize: 34, lineHeight: 1, filter: `drop-shadow(0 2px 8px ${neonGlow}88)` }}>{sp.emoji}</div>
                  <div style={{ position: 'absolute', bottom: 10, left: 0, right: 0, zIndex: 3, textAlign: 'center', fontSize: 9, fontWeight: 800, letterSpacing: '.8px', textTransform: 'uppercase', color: 'rgba(255,255,255,.7)' }}>{sp.label}</div>
                </>
              )}
            </div>
            {bday && (
              <>
                ...
                <div style={{ position: 'absolute', top: -18, left: '50%', transform: 'translateX(-52%) rotate(-13deg)', fontSize: 52, zIndex: 30, lineHeight: 1, filter: 'drop-shadow(0 3px 6px rgba(0,0,0,.5))', animation: 'capBob 2.2s ease-in-out infinite', pointerEvents: 'none' }}>🎉</div>
                <div style={{ position: 'absolute', left: 0, right: 0, bottom: 28, zIndex: 9, padding: '5px 4px', background: 'linear-gradient(90deg,#f97316,#ec4899,#a855f7,#3b82f6,#ec4899,#f97316)', backgroundSize: '200% 100%', animation: 'ribbonSlide 2s linear infinite', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 4, boxShadow: '0 2px 10px rgba(0,0,0,.4)' }}>
                  <span style={{ fontSize: 11 }}>🎀</span>
                  <span style={{ fontSize: 9, fontWeight: 900, color: '#fff', letterSpacing: '1.2px', textTransform: 'uppercase', textShadow: '0 1px 3px rgba(0,0,0,.5)', whiteSpace: 'nowrap' }}>Happy Birthday!</span>
                  <span style={{ fontSize: 11 }}>🎀</span>
                </div>
              </>
            )}
            ...
            <div className="photo-hover-overlay" style={{ position: 'absolute', inset: 0, zIndex: 20, background: 'rgba(0,0,0,.55)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 5, opacity: 0, transition: 'opacity .2s', cursor: 'pointer' }}
              onClick={e => { e.stopPropagation(); fileRef.current?.click(); }}>
              <div style={{ width: 32, height: 32, borderRadius: '50%', background: 'rgba(255,255,255,.2)', border: '1.5px solid rgba(255,255,255,.6)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 14 }}>{uploading ? '⏳' : '📷'}</div>
              <span style={{ fontSize: 9, fontWeight: 700, color: '#fff', textTransform: 'uppercase', letterSpacing: '.4px' }}>{uploading ? 'Uploading...' : localPhoto ? 'Change Photo' : 'Add Photo'}</span>
            </div>
            <input ref={fileRef} type="file" accept="image/*" style={{ display: 'none' }} onChange={handlePhotoChange} />
          </div>
          <div style={{ flex: 1, padding: '13px 14px', background: bday ? 'linear-gradient(160deg,#fff9f5,#fdf4ff,#f0f9ff)' : '#fff', display: 'flex', flexDirection: 'column', justifyContent: 'center', gap: 4 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 7, flexWrap: 'wrap', marginBottom: 1 }}>
              ...
              {isPrimary && <span style={{ fontSize: 9, fontWeight: 800, letterSpacing: '.5px', textTransform: 'uppercase', padding: '2px 7px', borderRadius: 5, color: '#fff', background: sp.neon }}>Primary</span>}
              {bday && <span style={{ fontSize: 10, fontWeight: 800, color: '#fff', background: 'linear-gradient(90deg,#f97316,#ec4899)', padding: '3px 10px', borderRadius: 20, animation: 'chipPulse 1.5s ease-in-out infinite' }}>🎂 Today!</span>}
            </div>
            ...
            {bday && <div style={{ fontSize: 12, color: '#9a3412', background: 'linear-gradient(135deg,#fff7ed,#fdf2f8)', border: '1px solid #fed7aa', borderRadius: 9, padding: '7px 10px', lineHeight: 1.5, fontWeight: 500, marginBottom: 4 }}>🎉 Happy Birthday {pet.petName}! We picked allergy-safe treats just for them.</div>}
            {pet.allergies && pet.allergies.length > 0 ? (
              <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap' }}>
                {pet.allergies.slice(0, 3).map(a => <span key={a} style={{ display: 'inline-flex', alignItems: 'center', gap: 2, background: '#fef2f2', color: '#dc2626', border: '1px solid #fecaca', borderRadius: 5, fontSize: 10, fontWeight: 700, padding: '2px 6px' }}>⚠ {a}</span>)}
                {pet.allergies.length > 3 && <span style={{ fontSize: 10, fontWeight: 700, color: '#dc2626', background: '#fef2f2', border: '1px solid #fecaca', borderRadius: 5, padding: '2px 6px' }}>+{pet.allergies.length - 3}</span>}
              </div>
            ) : (
              <span style={{ fontSize: 10, fontWeight: 600, color: '#16a34a', background: '#f0fdf4', border: '1px solid #bbf7d0', borderRadius: 5, padding: '2px 8px', alignSelf: 'flex-start' }}>No allergens</span>
            )}
        ...
        {expanded && (
          <div style={{ padding: '14px 16px 16px', background: '#fafbfc', borderTop: '1px solid #f1f5f9' }}>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 8, marginBottom: 12 }}>
              {[{ label: 'Birthday', val: pet.birthday ? new Date(pet.birthday).toLocaleDateString('en-US', { month: 'short', day: 'numeric' }) : '—' }, { label: 'Weight', val: pet.weight ? `${pet.weight} lbs` : '—' }, { label: 'Size', val: pet.size || '—' }, { label: 'Sex', val: pet.sex && pet.sex !== 'unknown' ? (pet.sex === 'male' ? 'Male ♂' : 'Female ♀') : '—' }, { label: 'Fixed', val: pet.isFixed === true ? 'Yes ✓' : pet.isFixed === false ? 'No' : '—' }, { label: 'Allergies', val: pet.allergies?.length ? `${pet.allergies.length} item${pet.allergies.length > 1 ? 's' : ''}` : 'None' }].map(({ label, val }) => (
            ...
            <div style={{ display: 'flex', gap: 8 }}>
              <button onClick={e => { e.stopPropagation(); onEdit(); }} style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, background: '#1e3a8a', color: '#fff', border: 'none', borderRadius: 9, padding: 10, fontSize: 12, fontWeight: 700, cursor: 'pointer' }}>✏️ Edit {pet.petName}'s Profile</button>
              <button onClick={e => { e.stopPropagation(); onDelete(); }} style={{ background: '#fef2f2', color: '#dc2626', border: '1px solid #fecaca', borderRadius: 9, padding: '10px 14px', fontSize: 12, fontWeight: 700, cursor: 'pointer' }}>🗑 Remove</button>
            </div>
          </div>
        )}
      </div>
...
function EmptyPetState({ onAdd }: { onAdd: () => void }) {
  const trio = ['dog', 'cat', 'bird'] as const;
  return (
    <div className="flex flex-col items-center text-center py-16 px-6">
      <div className="relative mb-8" style={{ width: 130, height: 72 }}>
        {trio.map((s, i) => (
          <div key={s} className="absolute" style={{ left: i * 34, top: 0, width: 72, height: 72, borderRadius: 20, background: SP[s].deepGrad, border: '1px solid rgba(255,255,255,0.12)', boxShadow: `0 8px 32px rgba(0,0,0,0.5), 0 0 20px ${SP[s].neon}33`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 32, zIndex: i }}>{SP[s].emoji}</div>
        ))}
      </div>
      <h4 style={{ fontSize: 22, fontWeight: 900, color: 'white', letterSpacing: '-0.02em', marginBottom: 8 }}>Build your pet family</h4>
      <p style={{ fontSize: 13, color: 'rgba(255,255,255,0.35)', marginBottom: 28, maxWidth: 260, lineHeight: 1.6 }}>Add your pets once. We handle the rest — personalized picks, birthday alerts, allergen warnings.</p>
      <button onClick={onAdd} className="flex items-center gap-2 font-black text-sm px-8 py-3 rounded-2xl transition-all hover:scale-105" style={{ ...glassBtn, border: '1px solid rgba(255,255,255,0.18)', color: 'white', fontSize: 13 }}>
...
const Profile = () => {
  ...
  const addPetMutation = useMutation({ mutationFn: petService.addPet, onSuccess: p => { queryClient.invalidateQueries({ queryKey: ['myPets'] }); setShowAddForm(false); showToast(`${p.petName} added to your family 🐾`, 'success'); }, onError: (e: any) => showToast(e.response?.data?.message || 'Failed to add', 'error') });
  const updatePetMutation = useMutation({ mutationFn: ({ id, data }: { id: string; data: any }) => petService.updatePet(id, data), onSuccess: p => { queryClient.invalidateQueries({ queryKey: ['myPets'] }); setEditingPetId(null); showToast(`${p.petName} updated`, 'success'); }, onError: (e: any) => showToast(e.response?.data?.message || 'Failed', 'error') });
  const deletePetMutation = useMutation({ mutationFn: petService.deletePet, onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['myPets'] }); setDeletingPetId(null); showToast('Removed', 'success'); }, onError: (e: any) => showToast(e.response?.data?.message || 'Failed', 'error') });
  const updateProfileMutation = useMutation({ mutationFn: authService.updateProfile, onSuccess: u => { setUser(u); setIsEditingProfile(false); showToast('Profile updated', 'success'); }, onError: (e: any) => showToast(e.response?.data?.message || 'Update failed', 'error') });
  const updatePasswordMutation = useMutation({ mutationFn: ({ currentPassword, newPassword }: { currentPassword: string; newPassword: string }) => authService.updatePassword(currentPassword, newPassword), onSuccess: () => { setIsChangingPassword(false); setPasswordData({ currentPassword: '', newPassword: '', confirmPassword: '' }); showToast('Password updated', 'success'); }, onError: (e: any) => showToast(e.response?.data?.message || 'Failed', 'error') });
  ...
  const toData = (f: typeof EMPTY_FORM) => ({ petName: f.petName, species: f.species, breed: f.breed || undefined, birthday: f.birthday || undefined, sex: (f.sex as any) || undefined, isFixed: f.isFixed || undefined, weight: f.weight ? Number(f.weight) : undefined, size: (f.size as any) || undefined, allergies: f.allergies.length ? f.allergies : undefined, notes: f.notes || undefined });
          ...
          <div className="lg:col-span-2 space-y-6">
            <div className="bg-white rounded-2xl shadow-md p-6">
              ...
              <p className="text-sm text-gray-600 mb-4">
                Pick Ask first (10% off, max $10) or Autoship (7% off, max $10). Add or change food and treats. We never charge unless you pay.
              </p>
            ...
            <>
              <style>{`
                @keyframes confettiFall { 0% { transform: translateY(-10px) rotate(0deg); opacity: 1; } 100% { transform: translateY(165px) rotate(720deg); opacity: 0; } }
                @keyframes balloonFloat { 0%, 100% { transform: translateY(0); } 50% { transform: translateY(-20px); } }
                @keyframes sparkle { 0%, 100% { opacity: 0; transform: scale(.7); } 50% { opacity: 1; transform: scale(1.2); } }
                @keyframes bdayBorder { 0%, 100% { background-position: 0% 50%; } 50% { background-position: 100% 50%; } }
                @keyframes ribbonSlide { 0% { background-position: 0% center; } 100% { background-position: 200% center; } }
                @keyframes capBob { 0%, 100% { transform: translateX(-52%) rotate(-13deg) translateY(0); } 50% { transform: translateX(-52%) rotate(-13deg) translateY(-5px); } }
                @keyframes chipPulse { 0%, 100% { opacity: 1; transform: scale(1); } 50% { opacity: .85; transform: scale(1.04); } }
                .photo-hover-overlay:hover { opacity: 1 !important; }
              `}</style>

              {editingPetId && (
                <PetForm initial={petToForm(pets.find(p => p._id === editingPetId)!)} onSubmit={f => updatePetMutation.mutate({ id: editingPetId, data: toData(f) })} onCancel={() => setEditingPetId(null)} isPending={updatePetMutation.isPending} isEdit />
              )}
              {showAddForm && !editingPetId && (
                <PetForm onSubmit={f => addPetMutation.mutate(toData(f) as any)} onCancel={() => setShowAddForm(false)} isPending={addPetMutation.isPending} />
              )}
              ...
              <div style={{ borderRadius: 20, background: '#fff', boxShadow: '0 2px 16px rgba(15,23,42,.07)', border: '1.5px solid #f1f5f9' }}>
                <div style={{ padding: '18px 20px 14px', borderBottom: '1.5px solid #f1f5f9', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                    ...
                    {pets.length > 0 && <span style={{ fontSize: 10, fontWeight: 800, color: '#64748b', background: '#f1f5f9', border: '1px solid #e2e8f0', borderRadius: 20, padding: '2px 9px' }}>{pets.length}</span>}
                  </div>
                  {pets.length > 0 && pets.length < 10 && (
                    <button onClick={() => { setShowAddForm(true); setEditingPetId(null); }} style={{ display: 'flex', alignItems: 'center', gap: 6, background: '#1e3a8a', color: '#fff', border: 'none', borderRadius: 10, padding: '8px 14px', fontSize: 12, fontWeight: 800, cursor: 'pointer' }}>
                      <Plus size={12} /> Add Pet
                    </button>
                  )}
                </div>
                <div style={{ padding: '8px 20px', borderBottom: '1px solid #f8fafc', background: '#fafbfc' }}>
                  <p style={{ fontSize: 11, color: '#94a3b8', fontWeight: 500, margin: 0 }}>Personalized picks, birthday alerts &amp; allergen warnings — for every pet.</p>
                </div>

                {pets.length === 0 && <EmptyPetState onAdd={() => setShowAddForm(true)} />}
                ...
                {pets.length > 0 && (
                  <div style={{ padding: '14px 14px 6px', display: 'flex', flexDirection: 'column', gap: 10 }}>
                    {pets.map((pet, idx) => (
                      deletingPetId === pet._id ? (
                        <div key={pet._id} style={{ borderRadius: 14, background: '#fef2f2', border: '1.5px solid #fecaca', padding: '16px 18px', display: 'flex', alignItems: 'center', gap: 14 }}>
                          ...
                          <div style={{ display: 'flex', gap: 8 }}>
                            <button onClick={() => deletePetMutation.mutate(pet._id)} disabled={deletePetMutation.isPending} style={{ background: '#dc2626', color: '#fff', border: 'none', borderRadius: 9, padding: '8px 16px', fontSize: 12, fontWeight: 800, cursor: 'pointer', opacity: deletePetMutation.isPending ? .5 : 1 }}>{deletePetMutation.isPending ? '...' : 'Remove'}</button>
                            <button onClick={() => setDeletingPetId(null)} style={{ background: '#fff', color: '#64748b', border: '1.5px solid #e2e8f0', borderRadius: 9, padding: '8px 16px', fontSize: 12, fontWeight: 700, cursor: 'pointer' }}>Keep</button>
                          </div>
                        </div>
                      ) : (
                    ...
                    {pets.length < 10 && (
                      <button onClick={() => { setShowAddForm(true); setEditingPetId(null); }} style={{ width: '100%', padding: '13px', borderRadius: 14, border: '2px dashed #e2e8f0', background: 'transparent', color: '#94a3b8', fontSize: 13, fontWeight: 700, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, marginTop: 2, boxSizing: 'border-box' }}>
                ...
                {pets.length > 0 && (
                  <div style={{ padding: '10px 20px 14px', marginTop: 8, borderTop: '1px solid #f8fafc', display: 'flex', gap: 20, flexWrap: 'wrap' }}>
                    <span style={{ fontSize: 11, color: '#94a3b8', fontWeight: 600 }}>🐾 {pets.length} pet{pets.length !== 1 ? 's' : ''} registered</span>
                    {[...new Set(pets.flatMap(p => p.allergies || []))].length > 0 && <span style={{ fontSize: 11, color: '#94a3b8', fontWeight: 600 }}>⚠ {[...new Set(pets.flatMap(p => p.allergies || []))].length} allergen{[...new Set(pets.flatMap(p => p.allergies || []))].length !== 1 ? 's' : ''} tracked</span>}
                    {pets.some(p => isBirthdayToday(p.birthday)) && <span style={{ fontSize: 11, color: '#9a3412', fontWeight: 700 }}>🎂 Birthday today!</span>}
                  </div>
                )}
              </div>
            </>
              ...
              {!isChangingPassword ? (
                ...
                <form onSubmit={e => { e.preventDefault(); if (passwordData.newPassword !== passwordData.confirmPassword) { showToast("Passwords don't match", 'error'); return; } if (passwordData.newPassword.length < 8) { showToast('Min 8 characters', 'error'); return; } updatePasswordMutation.mutate({ currentPassword: passwordData.currentPassword, newPassword: passwordData.newPassword }); }} className="space-y-4">
                  {[['Current Password', 'currentPassword'], ['New Password', 'newPassword'], ['Confirm New Password', 'confirmPassword']].map(([label, key]) => (
                    <div key={key}><label className="block text-sm font-medium mb-1.5">{label} *</label><input type="password" required value={passwordData[key as keyof typeof passwordData]} onChange={e => setPasswordData({ ...passwordData, [key]: e.target.value })} minLength={key !== 'currentPassword' ? 8 : undefined} className={inputCls} /></div>
                  ))}

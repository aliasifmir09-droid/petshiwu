...
const Cart = () => {
  ...
  const loadSharedCart = async (shareId: string) => {
    try {
      ...
      if (response.success && response.data.items) {
        // Convert shared cart items to cart store format
        // Note: This would require product details, so we'll just show a message
  ...
  const { data: deliveryData } = useQuery({
    ...
    queryFn: () => cartService.getDeliveryEstimate('standard'),
  ...
  // One-click checkout (for authenticated users with previous orders)
  const handleOneClickCheckout = () => {
    ...
    // Navigate to checkout with quick checkout flag
          ...
          <div className="bg-white rounded-lg shadow p-6 sticky top-24">
            ...
            {isAuthenticated && (
              <button
                ...
                title="Quick checkout for returning customers"
              >
                <Zap size={18} />
                One-Click Checkout
              </button>
      ...
      <ConfirmationModal
        ...
        icon={
          <div className={`w-16 h-16 rounded-full flex items-center justify-center ${
            confirmModal.action === 'save-for-later' ? 'bg-blue-100' : 'bg-red-100'
          }`}>
            {confirmModal.action === 'save-for-later' ? (

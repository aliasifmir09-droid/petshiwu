...
const MAX_IMAGES  = 1;  // only first Scene7 ID — secondary IDs are often wrong product/badge images
...
async function processOne(db, id, name, existingImages) {
  ...
  if (bunnyImages.length >= 3) return 'skip';
  ...
  try {
    ...
    // Merge: keep existing Bunny images + add new ones (deduped by filename stem)
    const existingStem = new Set(bunnyImages.map(u => u.split('/').pop()));
    const merged = [...bunnyImages];
    for (const u of cdnUrls) {
      if (!existingStem.has(u.split('/').pop())) merged.push(u);
    }
    const finalImages = merged.slice(0, 5); // cap at 5
    await db.collection('products').updateOne(
      { _id: id },
      { $set: { images: finalImages } }
    );
    return `ok:${finalImages.length}`;
  } catch (e) {
